<template>
  <div class="vendor-orders-section">
    <div class="pz-admin-card pz-section-shell">
      <div class="pz-admin-card__header pz-section-shell__header">
        <div>
          ...
          <h3 class="pz-admin-card__title pz-section-shell__title">LOGISTICS_COMMAND_QUEUE</h3>
          <div class="pz-section-shell__meta">Fulfillment, shipment, and buyer response control.</div>
        </div>
        <Button variant="ghost" size="sm" @click="fetchOrders">REFRESH_STREAM</Button>
      </div>

      <div v-if="loading" class="pz-section-shell__content">
        <div class="pz-loading-state">
          <div class="pz-loading-state__indicator"></div>
          <div class="pz-loading-state__label">SYNCING_LOGISTICS_DATAFEED</div>
        </div>
      </div>
      ...
      <div v-else class="pz-order-stream pz-section-shell__content">
        <div v-for="order in orders" :key="order.id" class="pz-order-node pz-u-transition-base">
          <div class="pz-order-node__header pz-l-flex pz-l-flex--justify-between pz-l-flex--align-start u-mb-6">
            <div>
              <div class="pz-u-text-mono text-xs pz-u-color-concrete u-mb-1 font-bold">SEQUENCE_ID: #{{ order.id }}
              </div>
              <div class="pz-u-text-display text-sm">{{ order.buyer_name?.toUpperCase() || 'ANONYMOUS_BUYER' }}</div>
              <div class="pz-u-text-mono text-xs pz-u-color-concrete u-mt-1">
                TIMESTAMP: {{ new Date(order.created_at).toLocaleString().toUpperCase() }}
              </div>
            </div>
          ...
          <div class="pz-l-flex pz-l-flex--justify-end pz-l-flex--gap-3">
            <template v-if="order.status !== 'COMPLETED' && order.status !== 'CANCELLED'">
              <Button v-if="order.status === 'PLACED'" size="sm" variant="primary"
                @click="openFulfillmentModal(order, 'CONFIRMED')">CONFIRM_ORDER</Button>
              <Button v-if="order.status === 'CONFIRMED'" size="sm" variant="primary"
                @click="openFulfillmentModal(order, 'PACKING')">START_PACKING</Button>
              <Button v-if="order.status === 'PACKING'" size="sm" variant="primary"
                @click="openFulfillmentModal(order, 'SHIPPED')">INITIALIZE_SHIPMENT</Button>
              <Button v-if="order.status === 'SHIPPED'" size="sm" variant="primary"
                @click="openFulfillmentModal(order, 'DELIVERED')">MARK_DELIVERED</Button>
              <Button v-if="order.tracking_number" size="sm" variant="secondary"
                @click="openTracking(order.tracking_number)">TRACK_ORDER</Button>
            </template>
            <Button variant="ghost" size="sm" @click="openChat(order.id)">CHAT_WITH_BUYER</Button>
            <Button variant="ghost" size="sm">PRINT_MANIFEST</Button>
          </div>
        </div>
      </div>
    </div>
    ...
    <Modal :isOpen="showFulfillmentModal" title="OPS_STATUS_UPDATE_PROTOCOL" size="md"
      ...
      <form id="fulfillment-form" @submit.prevent="submitFulfillment"
        ...
        <div class="pz-u-bg-limestone pz-p-4 pz-u-border" style="border-left: 4px solid var(--pz-color-earth-orange);">
          <div class="pz-u-text-mono text-xs pz-u-color-concrete u-mb-1">TRANSITIONING_RESOURCE_TO:</div>
        ...
        <div v-if="fulfillmentForm.status === 'CONFIRMED'" class="pz-l-flex pz-l-flex--column pz-l-flex--gap-4">
          <PzInput type="date" v-model="fulfillmentForm.estimated_delivery_at" label="ESTIMATED_DELIVERY_ALPHA" required />
          <PzInput v-model="fulfillmentForm.notes" label="LOGISTICS_NOTES" />
        </div>

        <div v-if="fulfillmentForm.status === 'SHIPPED'" class="pz-l-flex pz-l-flex--column pz-l-flex--gap-4">
          <div class="pz-input-wrapper">
            <label class="pz-input__label">SELECT_LOGISTICS_CARRIER</label>
          ...
          <div class="pz-u-text-mono text-xs pz-u-color-concrete">
            // SYSTEM_WILL_GENERATE_TRACKING_ID_ON_EXECUTION
          </div>
        </div>
      </form>
      <template #footer>
        <Button variant="ghost" @click="showFulfillmentModal = false">ABORT_PROTOCOL</Button>
        <Button type="submit" form="fulfillment-form" variant="primary" :loading="updating">EXECUTE_UPDATE</Button>
      </template>
    </Modal>

    <!-- Tracking Modal -->
    <Modal :isOpen="showTrackingModal" title="TACTICAL_SHIPMENT_MONITOR" size="xl" @close="showTrackingModal = false">
      <LogisticsTracker :trackingNumber="activeTrackingNumber" />
    </Modal>

    <!-- Chat Modal -->
    <Modal :isOpen="showChatModal" title="SECURE_BUYER_LINK" size="lg" @close="showChatModal = false">
